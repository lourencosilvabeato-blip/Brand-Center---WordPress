<?php
/**
 * Ascendum Brand Center — Theme Functions
 *
...
function abc_theme_setup() {
    ...
    register_nav_menus(
        array(
            'primary' => __( 'Primary Navigation', 'ascendum-brand-center' ),
        )
    );
}
...
function abc_register_acf_blocks() {
    ...
    //     'name'            => 'block-name',
    //     'title'           => __( 'Block Title', 'ascendum-brand-center' ),
    //     'description'     => __( 'Block description.', 'ascendum-brand-center' ),
...
function abc_add_user_reset_password_action( $actions, $user ) {
    ...
    $actions['abc_reset_password'] = '<a href="' . esc_url( $url ) . '">'
        . esc_html__( 'Reset Password', 'ascendum-brand-center' )
        . '</a>';
    return $actions;
}
...
function abc_handle_admin_password_reset() {
    ...
    if ( ! current_user_can( 'manage_options' ) ) {
        wp_die( esc_html__( 'Unauthorized.', 'ascendum-brand-center' ), '', array( 'response' => 403 ) );
    }
    ...
    if ( ! $user_id ) {
        wp_die( esc_html__( 'Invalid user ID.', 'ascendum-brand-center' ), '', array( 'response' => 400 ) );
    }

    $user = get_userdata( $user_id );
    if ( ! $user || ! in_array( 'external_user', (array) $user->roles, true ) ) {
        wp_die(
            esc_html__( 'Password reset is only available for external users.', 'ascendum-brand-center' ),
            '',
            array( 'response' => 400 )
        );
    }
...
function abc_admin_reset_password_notice() {
    ...
    <div class="notice notice-success is-dismissible">
        <p><?php esc_html_e( 'Password reset email sent successfully.', 'ascendum-brand-center' ); ?></p>
    </div>
    <?php
}
...
function abc_menu_item_search_filter_field( $item_id, $item, $depth, $args ) {
    ...
    <p class="field-move description description-wide" style="margin-top:6px;">
        <label>
            ...
            >
            <?php esc_html_e( 'Show as search filter on homepage', 'ascendum-brand-center' ); ?>
        </label>
    </p>
    <?php
}
